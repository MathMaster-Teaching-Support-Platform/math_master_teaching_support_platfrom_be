...
public interface SubmissionRepository extends JpaRepository<Submission, UUID> {
    ...
    @Query("SELECT s FROM Submission s JOIN s.assessment a WHERE a.teacherId = :teacherId " +
    ...
    @Query("SELECT s FROM Submission s WHERE s.assessmentId = :assessmentId ORDER BY s.submittedAt DESC")
    List<Submission> findAllByAssessmentId(@Param("assessmentId") UUID assessmentId);

    // Count submissions by status for an assessment
    @Query("SELECT COUNT(s) FROM Submission s WHERE s.assessmentId = :assessmentId AND s.status = :status")
    Long countByAssessmentIdAndStatus(@Param("assessmentId") UUID assessmentId, @Param("status") SubmissionStatus status);
}
