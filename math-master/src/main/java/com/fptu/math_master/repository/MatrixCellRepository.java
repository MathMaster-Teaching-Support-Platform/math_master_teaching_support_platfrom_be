...
public interface MatrixCellRepository extends JpaRepository<MatrixCell, UUID> {
  ...
  @Query(
      "SELECT SUM(mc.numQuestions * mc.pointsPerQuestion) FROM MatrixCell mc WHERE mc.matrixId = :matrixId")
  Double sumPointsByMatrixId(@Param("matrixId") UUID matrixId);

  @Query(
      "SELECT mc.difficulty, SUM(mc.numQuestions) FROM MatrixCell mc "
          + "WHERE mc.matrixId = :matrixId GROUP BY mc.difficulty")
  List<Object[]> getDifficultyDistribution(@Param("matrixId") UUID matrixId);

  @Query(
      "SELECT mc.cognitiveLevel, SUM(mc.numQuestions) FROM MatrixCell mc "
  ...
  @Query(
      "SELECT c.title, SUM(mc.numQuestions) FROM MatrixCell mc "
  ...
  @Query(
      "SELECT COUNT(DISTINCT mc.cognitiveLevel) FROM MatrixCell mc WHERE mc.matrixId = :matrixId")
