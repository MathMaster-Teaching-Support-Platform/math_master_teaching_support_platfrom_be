...
public interface QuizAttemptRepository extends JpaRepository<QuizAttempt, UUID> {
    ...
    @Query("SELECT qa FROM QuizAttempt qa WHERE qa.submissionId = :submissionId ORDER BY qa.attemptNumber DESC")
    List<QuizAttempt> findBySubmissionIdOrderByAttemptNumberDesc(@Param("submissionId") UUID submissionId);

    @Query("SELECT qa FROM QuizAttempt qa WHERE qa.submissionId = :submissionId AND qa.attemptNumber = :attemptNumber")
    Optional<QuizAttempt> findBySubmissionIdAndAttemptNumber(
        @Param("submissionId") UUID submissionId,
        @Param("attemptNumber") Integer attemptNumber
    );

    @Query("SELECT qa FROM QuizAttempt qa WHERE qa.assessmentId = :assessmentId AND qa.studentId = :studentId " +
