...
@Repository
public interface MatrixQuestionMappingRepository
    extends JpaRepository<MatrixQuestionMapping, UUID> {

  @Query(
      "SELECT mqm FROM MatrixQuestionMapping mqm WHERE mqm.matrixCellId = :matrixCellId ORDER BY mqm.selectionPriority")
  List<MatrixQuestionMapping> findByMatrixCellIdOrderByPriority(
      @Param("matrixCellId") UUID matrixCellId);
