...
public interface ExamMatrixRepository extends JpaRepository<ExamMatrix, UUID> {
  ...
  @Query("SELECT em FROM ExamMatrix em WHERE em.assessmentId = :assessmentId AND em.deletedAt IS NULL")
  Optional<ExamMatrix> findByAssessmentIdAndNotDeleted(@Param("assessmentId") UUID assessmentId);

  @Query("SELECT COUNT(em) > 0 FROM ExamMatrix em WHERE em.assessmentId = :assessmentId AND em.deletedAt IS NULL")
  ...
  @Query("SELECT COUNT(mc) FROM MatrixCell mc WHERE mc.matrixId = :matrixId AND mc.numQuestions > 0")
