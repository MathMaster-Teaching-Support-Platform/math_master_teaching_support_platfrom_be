...
public interface RegradeRequestRepository extends JpaRepository<RegradeRequest, UUID> {
  ...
  @Query(
      "SELECT r FROM RegradeRequest r WHERE r.submissionId = :submissionId ORDER BY r.createdAt DESC")
  ...
  Page<RegradeRequest> findByStatus(
      @Param("status") RegradeRequestStatus status, Pageable pageable);
