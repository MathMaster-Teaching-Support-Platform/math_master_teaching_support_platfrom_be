...
public interface ExamMatrixService {
  ...
  /**
   * FR-EM-NEW: List Matching Question Templates for a Matrix Cell. Validates matrixId/cellId
   * ownership, derives requirements from the cell, filters + ranks question templates, and returns
   ...
   *
   * <p>Generates in-memory candidate questions from a chosen template so the teacher can preview
   * quality before finalising. Nothing is persisted to any table.
   *
   * @param matrixId exam matrix ID
   * @param cellId matrix cell ID (must belong to matrixId)
   ...
   * <ol>
   *   <li>questions table
   *   <li>assessment_questions table
   *   <li>matrix_question_mapping table
   * </ol>
   *
