...
public class TemplateImportServiceImpl implements TemplateImportService {
    ...
    private TemplateImportResponse analyzeWithAI(String text, String subjectHint, String contextHint) {
        try {
    ...
    private String buildAnalysisPrompt(String text, String subjectHint, String contextHint) {
        ...
        prompt.append("# ROLE\n");
        prompt.append("You are a Mathematics Question Template Analyzer for an educational platform.\n");
        prompt.append("Your purpose is to analyze extracted text from uploaded files and propose a reusable Question Template DRAFT.\n");
        ...
        prompt.append("Step 4: Identify variables that could be parameterized (numbers, coefficients)\n");
        prompt.append("Step 5: Infer the mathematical structure (e.g., ax + b = c, quadratic, geometry)\n");
        ...
        prompt.append("- Preserve exact operator precedence: use parentheses (a + b) * c NOT a + b * c if ambiguous\n");
        ...
        prompt.append("    \"parameterizableElements\": [\"coefficient a\", \"constant b\", \"result c\"],\n");
        ...
        prompt.append("      {\"name\": \"a\", \"type\": \"integer\", \"min\": 1, \"max\": 10, \"description\": \"Coefficient of x\"},\n");
        prompt.append("      {\"name\": \"b\", \"type\": \"integer\", \"min\": -20, \"max\": 20, \"description\": \"Constant term (can be negative)\"},\n");
        prompt.append("      {\"name\": \"c\", \"type\": \"integer\", \"min\": 1, \"max\": 50, \"description\": \"Result value\"}\n");
        prompt.append("    ],\n");
        ...
        prompt.append("  },\n");
        prompt.append("  \"warnings\": [\"Parameter b can be negative - ensure display handles minus signs\", \"Division by zero if a=0\"]\n");
        prompt.append("}\n\n");

        prompt.append("# FIELD DEFINITIONS\n");
        prompt.append("- analysisSuccessful: boolean, true if analysis completed, false if text is unprocessable\n");
        prompt.append("- confidenceScore: 0.0 to 1.0, based on pattern clarity and mathematical validity\n");
        prompt.append("- detectedType: MULTIPLE_CHOICE | TRUE_FALSE | SHORT_ANSWER | ESSAY\n");
        prompt.append("- answerFormula: Mathematical expression using {{parameter}} syntax. MUST be algebraically correct.\n");
        prompt.append("- cognitiveLevel: REMEMBER | UNDERSTAND | APPLY | ANALYZE | EVALUATE | CREATE\n");
        prompt.append("- warnings: Array of strings. Include ANY mathematical risks, ambiguities, or edge cases.\n\n");
    ...
    private TemplateImportResponse parseAIAnalysis(String aiResponse, String originalText) {
        try {
            log.info("Parsing AI response, length: {} characters", aiResponse != null ? aiResponse.length() : 0);

            // Extract JSON from response
            String jsonContent = extractJSON(aiResponse);
            log.info("Extracted JSON content, length: {} characters", jsonContent != null ? jsonContent.length() : 0);
            ...
            if (templateNode.has("parameters") && templateNode.get("parameters").isArray()) {
                for (JsonNode param : templateNode.get("parameters")) {
                    placeholders.add(TemplateImportResponse.PlaceholderSuggestion.builder()
            ...
            TemplateImportResponse response = TemplateImportResponse.builder()
            ...
            log.error("AI response that failed to parse (first 1000 chars): {}",
                aiResponse != null ? aiResponse.substring(0, Math.min(1000, aiResponse.length())) : "null");
            return performRuleBasedAnalysis(originalText);
        }
    }

    private TemplateImportResponse postProcessAIResponse(TemplateImportResponse response, String originalText) {
        try {
            ...
            if (response.getSuggestedTemplate() != null) {
                ...
                if (response.getSuggestedTemplate().getTemplateType() == QuestionType.MULTIPLE_CHOICE &&
                    ...
                    defaultGenerator.put("distractors", List.of(
                ...
                if (formula != null && formula.contains("/ a")) {
                    ...
                    if (templateText2 != null) {
                        ...
                        if (templateStr.contains("{{a}}") &&
                            !additionalWarnings.stream().anyMatch(w -> w.contains("a=0") || w.contains("a != 0"))) {
                            additionalWarnings.add("CRITICAL: Add constraint 'a != 0' to prevent division by zero");
                        }
                    }
                }
                ...
                if (diffRules != null && diffRules.size() >= 2) {
                    ...
                    if (diffRules.containsKey("easy") && diffRules.containsKey("medium") && diffRules.containsKey("hard")) {
                        ...
                        if ((easy.contains("<=") && medium.contains("<=")) ||
                            ...
                            additionalWarnings.add("WARNING: Difficulty rules may overlap - verify mutual exclusivity");
                        }
                    }
                }
            }
    ...
    private String detectLanguage(String text) {
        ...
        if (text.matches(".*[àáảãạăằắẳẵặâầấẩẫậèéẻẽẹêềếểễệìíỉĩịòóỏõọôồốổỗộơờớởỡợùúủũụưừứửữựỳýỷỹỵđÀÁẢÃẠĂẰẮẲẴẶÂẦẤẨẪẬÈÉẺẼẸÊỀẾỂỄỆÌÍỈĨỊÒÓỎÕỌÔỒỐỔỖỘƠỜỚỞỠỢÙÚỦŨỤƯỪỨỬỮỰỲÝỶỸỴĐ].*")) {
            return "vi";
        }

        // Check for common Vietnamese words
        String lowerText = text.toLowerCase();
        if (lowerText.contains("giải") || lowerText.contains("phương trình") ||
            lowerText.contains("câu") || lowerText.contains("đề")) {
            return "vi";
        }
    ...
    private String extractJSON(String content) {
        ...
        log.debug("Extracting JSON from content: {}", content.substring(0, Math.min(200, content.length())) + "...");
