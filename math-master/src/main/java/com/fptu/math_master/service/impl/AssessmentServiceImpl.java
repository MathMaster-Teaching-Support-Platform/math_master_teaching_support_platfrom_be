...
public class AssessmentServiceImpl implements AssessmentService {
  ...
  public AssessmentResponse createAssessment(AssessmentRequest request) {
    ...
    Assessment assessment = Assessment.builder()
      ...
      .showCorrectAnswers(request.getShowCorrectAnswers() != null ? request.getShowCorrectAnswers() : false)
  ...
  public AssessmentResponse updateAssessment(UUID id, AssessmentRequest request) {
    ...
    Assessment assessment = assessmentRepository.findByIdAndNotDeleted(id)
  ...
  public AssessmentResponse setPointsOverride(UUID assessmentId, PointsOverrideRequest request) {
    log.info("Setting points override for assessment: {}, question: {}", assessmentId, request.getQuestionId());
    ...
    AssessmentQuestion aq = assessmentQuestionRepository
  ...
  public AssessmentSummary getPublishSummary(UUID id) {
    ...
    BigDecimal totalPoints = totalPointsDouble != null ? BigDecimal.valueOf(totalPointsDouble) : BigDecimal.ZERO;
    ...
    } else if (assessment.getStartDate() != null && assessment.getStartDate().isBefore(Instant.now())) {
      canPublish = false;
    ...
    return AssessmentSummary.builder()
      ...
      .startDate(assessment.getStartDate() != null ? formatter.format(assessment.getStartDate()) : null)
  ...
  @Override
  @Transactional(readOnly = true)
  public Page<AssessmentResponse> getMyAssessments(AssessmentStatus status, UUID lessonId, Pageable pageable) {
    log.info("Getting my assessments - status: {}, lessonId: {}", status, lessonId);

    UUID currentUserId = getCurrentUserId();
    Page<Assessment> assessments = assessmentRepository.findWithFilters(
      currentUserId,
      status,
      lessonId,
      pageable
  ...
  public boolean canEditAssessment(UUID id) {
    ...
    UUID currentUserId = getCurrentUserId();
    boolean isOwnerOrAdmin = assessment.getTeacherId().equals(currentUserId) || isAdmin(currentUserId);
  ...
  public boolean canDeleteAssessment(UUID id) {
    ...
    return isOwnerOrAdmin && assessment.getStatus() == AssessmentStatus.DRAFT && submissionCount == 0;
  }
  ...
  private AssessmentResponse mapToResponse(Assessment assessment) {
    ...
    String teacherName = userRepository.findById(assessment.getTeacherId())
      .map(User::getFullName)
      .orElse("Unknown");

    String lessonTitle = null;
    if (assessment.getLessonId() != null) {
      lessonTitle = lessonRepository.findById(assessment.getLessonId())
        .map(Lesson::getTitle)
        .orElse(null);
    }
