...
public class AIEnhancementServiceImpl implements AIEnhancementService {
    ...
    public AIEnhancedQuestionResponse enhanceQuestion(AIEnhancementRequest request) {
        ...
        try {
            ...
            // Parse the response
            AIEnhancedQuestionResponse enhancedResponse = parseAIResponse(
            ...
            return createFallbackResponse(request,
                Collections.singletonList("AI enhancement failed: " + e.getMessage()));
        }
    }

    @Override
    public boolean validateAIOutput(AIEnhancementRequest request, AIEnhancedQuestionResponse response) {
        List<String> errors = new ArrayList<>();

        // 1. Validate correct answer hasn't changed
        if (!isSameAnswer(request.getCorrectAnswer(), response.getCorrectAnswerKey(), response.getEnhancedOptions())) {
        ...
        if (response.getEnhancedQuestionText() == null || response.getEnhancedQuestionText().trim().isEmpty()) {
    ...
    private String buildEnhancementPrompt(AIEnhancementRequest request) {
        ...
        prompt.append("  \"distractorExplanations\": {\"B\": \"Common mistake: ...\", \"C\": \"...\", \"D\": \"...\"}\n");
        prompt.append("}\n");

        return prompt.toString();
    }

    private AIEnhancedQuestionResponse parseAIResponse(String aiContent, AIEnhancementRequest request) {
        try {
            ...
            AIEnhancedQuestionResponse response = AIEnhancedQuestionResponse.builder()
    ...
    private boolean isSameAnswer(String originalAnswer, String answerKey, Map<String, String> options) {
    ...
    private boolean isMathematicsContent(String content) {
        ...
        // Math keywords
        boolean hasMathKeywords = lowerContent.matches(".*\\b(calculate|solve|find|equals?|sum|difference|product|quotient|formula|equation|number|value|result|answer)\\b.*");

        // Math symbols
        boolean hasMathSymbols = content.matches(".*[+\\-*/=<>≤≥≠√∑∏∫].*") || content.matches(".*\\d+.*");

        // Not containing non-math subjects
        boolean noOtherSubjects = !lowerContent.matches(".*(history|literature|biology|chemistry|physics|geography|art|music).*");
    ...
    private boolean containsInappropriateContent(String content) {
        ...
        String[] inappropriateKeywords = {
            "violence", "weapon", "drug", "alcohol", "sexual", "hate",
            "racist", "discrimination", "offensive"
        };
