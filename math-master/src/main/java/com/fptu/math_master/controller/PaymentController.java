...
public class PaymentController {
  ...
  public ApiResponse<Void> handleWebhook(@RequestBody PayOSWebhookRequest webhookRequest) {
    ...
    try {
      ...
      return ApiResponse.<Void>builder().message("Webhook processed successfully").build();
      ...
      return ApiResponse.<Void>builder().message("Webhook received but processing failed").build();
    }
  }

  @Operation(
      summary = "Get order status",
      description = "Poll the status of a deposit order by orderCode. Use every 5s for PENDING orders.")
  @SecurityRequirement(name = "bearerAuth")
  @GetMapping("/order/{orderCode}/status")
  public ApiResponse<TransactionResponse> getOrderStatus(@PathVariable Long orderCode) {
    UUID userId = getCurrentUserId();
    return ApiResponse.<TransactionResponse>builder()
        .result(paymentService.getOrderStatus(orderCode, userId))
        .build();
  }
